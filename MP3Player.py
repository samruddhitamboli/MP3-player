...
try:
    ...
    favlist = [s[:-1] for s in F.readlines()]


playlist = tkr.Listbox(player,width=500,height=300,highlightcolor="blue",selectmode=tkr.SINGLE,yscrollcommand=scrollbar.set)
pos = 0
for i in songlist:
...
def start_count(t):
    t1=0
    while t1!=t and pygame.mixer.music.get_busy():
        if paused:
            continue
        else:
...
def search_song():
    global playlist, ent_bx, songlist
    songname = ent_bx.get()
    playlist.delete(0, 'end')
    p = 0
    if songname != "":
        for i in songlist:
            if i.rfind(songname) != -1:
                playlist.insert(p, i)
                p += 1
    else:
        for i in songlist:
...
ent_bx = tkr.Entry(f)
search_button = tkr.Button(f, text = "go!", command = search_song, background = "#55F")
